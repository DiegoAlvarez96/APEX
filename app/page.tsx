...
export default function Home() {
  ...
  const screen = {
    ...
    settings: <SettingsView settings={store.settings} onUpdateSettings={(settings) => void store.updateSettings(settings)} onExport={store.exportData} onOpenTrainingSettings={() => setTrainingSettingsOpen(true)} />,
  ...
  return (
    <main data-module={view} className="apex-shell mx-auto min-h-dvh w-full max-w-xl px-3 pb-20 pt-[calc(env(safe-area-inset-top)+10px)] lg:max-w-3xl" style={moduleStyle}>
      <AppHeader active={view} onNavigate={navigate} onRefresh={store.refresh} />
      ...
      <AnimatePresence>
        {trainingSettingsOpen ? (
          <motion.div className="fixed inset-0 z-[90] overflow-y-auto bg-[#0d0d0d] px-3 pb-6 pt-[calc(env(safe-area-inset-top)+12px)]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <SportSettingsPanel
              sports={store.sportProfiles}
              onAdd={store.addSportProfile}
              onUpdate={store.updateSportProfile}
              onDelete={(id) => void store.deleteSportProfile(id)}
              onDuplicate={store.duplicateSportProfile}
              onOpenAgenda={() => {
