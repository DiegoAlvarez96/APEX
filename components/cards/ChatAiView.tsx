...
export function ChatAiView({ messages, aiStatus, onSend, onNewChat }: { messages: ChatMessage[]; aiStatus: "available" | "offline" | "checking"; onSend: (content: string) => Promise<void> | void; onNewChat: () => Promise<void> | void }) {
  ...
  const status = loading ? { label: "Consultando OpenAI...", className: "bg-sky-400" } : aiStatus === "available" ? { label: "OpenAI disponible.", className: "bg-limeglass" } : { label: "Sin conexion.", className: "bg-white/45 light:bg-black/35" };
